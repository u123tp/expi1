...
{
	const char* usage =
		"Usage for client: %s -c <server_ip> [port (optional 50000 if not "
		"specified)] (or Usage for server: %s -s [port (optional 50000 if not "
		"specified)])\n";
	// API仕様：エラー時に返す
	...
	if (pid == 0) {	 // 子プロセス: 受信した音声を play に流す
		...
		while (1) {
			...
			fflush(play_fp);
			// fflush:
			// ファイルをフラッシュ(バッファリングしているものをplayに送信する)する
			// 引数:フラッシュするファイル
			// 返り値:0 : 成功，-1 : 失敗
		}
		pclose(play_fp);
		...
		while (1) {
			...
			while (sent < n) {	// 送信中
				...
				// send: データを送信する
				// 引数:送信するデータ, 送信するデータのサイズ,
				// 送信するデータの個数, 送信するデータのフラグ
